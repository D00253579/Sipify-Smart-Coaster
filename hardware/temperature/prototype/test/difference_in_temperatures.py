...
def write_to_file(contact_temp, submergible_temp):
    with open('temperature_difference_test.txt', 'a') as file:
        file.write(f"Contact Temp: {contact_temp} | Submergible Temp: {submergible_temp} | Timestamp: {time.strftime('%H:%M:%S')}\n")
...
while True:
    ...
    print(f"Contact Temp: {contact_temp}")
    print(f"Submergible Temp: {submergible_temp}")
